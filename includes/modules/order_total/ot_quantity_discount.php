...
class ot_quantity_discount {
      ...
      if ($cart_array == true) {
         ...
         while (list($keypos, $listpos) = each($key_list)) {
            if ($this->total_basis == 'Total By Category') {
               ...
               $this->apply_special_category_discount($listpos, $count, $disc_amount);
            }
            else {
               ...
               $this->apply_special_item_discount($listpos, $count, $disc_amount);
            }
            ...
            if ($this->units == 'currency') {
               $this->explanation .= " " . $this->print_amount($this_discount) . OFF . $count . " " . $description . "@" . $this->print_amount($price) . $gross_expl . "\\n";
            }
            else if ($this->units == 'currency per item') {
               $this->explanation .= " " . $this->print_amount($disc_amount) . PER_ITEM_OFF . $count . " " . $description . "@" . $this->print_amount($price) . $gross_expl . "\\n";
            }
            else {
               $this->explanation .= " " . $count . " " . $description . "@" . $this->print_amount($price) . " * " . $disc_amount . "% = " . $this->print_amount($this_discount) . $gross_expl . "\\n";
            }
         }
      }
      else {
         ...
         if ($this->units == 'currency') {
            $this->explanation .= " " . $this->print_amount($this_discount) . OFF . $count . " " . ITEMS . " @ " . $this->print_amount($price) . $gross_expl . "\\n";
         }
         else if ($this->units == 'currency per item') {
            $this->explanation .= " " . $this->print_amount($disc_amount) . PER_ITEM_OFF . $count . " " . ITEMS . " @ " . $this->print_amount($price) . $gross_expl . "\\n";
         }
         else {
            $this->explanation .= " " . $count . " " . ITEMS . " @ " . $this->print_amount($price) . " * " . $disc_amount . "% = " . $this->print_amount($this_discount) . $gross_expl . "\\n";
         }
      }
   ...
   function pre_confirmation_check(/** @noinspection PhpUnusedParameterInspection */
      $order_total) {
      $od_amount = $this->calculate_deductions();
   ...
   function get_html_policy(/** @noinspection PhpUnusedParameterInspection */
      $product = 0, /** @noinspection PhpUnusedParameterInspection */
      $category = 0) {
      $h_exp = "<br />";
   ...
   // Users seeking a pre-formatted html string should use get_html_policy()
   function get_discount_info(/** @noinspection PhpUnusedParameterInspection */
   ...
   function get_discount_parms($naked = false, /** @noinspection PhpUnusedParameterInspection */
                               $product = 0, /** @noinspection PhpUnusedParameterInspection */
                               $category = 0) {
      $response_arr = array();
      $pos = 0;
